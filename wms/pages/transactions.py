...
def _apply_filters(df: pd.DataFrame, locations_col) -> pd.DataFrame:
    ...
    with st.container(border=True):
        ...
        selected_skus = c1.multiselect(
            ...
            options=sku_options,
            default=[],
            help="Search and select one or more SKUs. Leave empty to include all SKUs.",
        )
        ...
        selected_locations = c4.multiselect(
            ...
            default=[],
            help="Search and select one or more Locations. Leave empty to include all Locations.",
        )
        ...
        min_date = ts.min().date() if ts is not None and ts.notna().any() else datetime.now().date()
    ...
    if show_fba_only:
        ...
        if "location_from" in out.columns:
            out["location_from"] = out["location_from"].astype(str).str.strip().str.upper()
        ...
        if "reason" in out.columns:
            ...
            location_from_is_amazon = out["location_from"].eq(amazon) if "location_from" in out.columns else False
...
def render(*, inventory_col, transactions_col, mm_col, locations_col) -> None:
    ...
    if "show_shipment_record" not in st.session_state:
    ...
    desired_cols = [
        "timestamp", "sku", "product_name", "shipment_id", "location",
        "type", "reason", "sto", "location_from", "location_to", "qty",
    ]
    ...
    inv_map = {
        ...
        for d in inventory_col.find({}, {"_id": 0, "sku": 1, "location": 1, "product_name": 1})
    }
    ...
    if "timestamp" in df.columns:
        df["timestamp"] = pd.to_datetime(df["timestamp"], errors="coerce").apply(utc_to_central)
    ...
    if st.session_state.show_shipment_record:
        if col_btn_2.button("✖️ Close Record"):
            st.session_state.show_shipment_record = False
            st.rerun()
        ...
        if not outbound_df.empty:
            ...
            if shipment_ids:
                ...
                for tracking in tracking_list:
                    item_length = len(tracking) + (2 if current_line else 0)
                ...
                # --- USPS Web Tracking Button (Synced with Page) ---
                if current_items:
                ...
                st.warning("No valid USPS tracking numbers found.")
        else:
